...
const StateMP = State{ADVI.MPA{Float64, 1},
                      ADVI.MPA{Float64, 2},
                      ADVI.MPA{Float64, 3}}
...
function State(c::Constants)
  ...
  s.delta0 = ADVI.ModelParam(c.L[0], "positive",
                             m=ones(c.L[0]), s=ones(c.L[0]))
  s.delta1 = ADVI.ModelParam(c.L[1], "positive",
                             m=ones(c.L[1]), s=ones(c.L[1]))
  s.eps = ADVI.ModelParam(c.I, "unit", m=fill(log(1e-6), c.I),
                          s=fill(1e-7, c.I))
  s.sig2 = ADVI.ModelParam(c.I, "positive", m=fill(log(.1), c.I),
                           s=fill(.1, c.I))
...
function rsample(s::StateMP, y::Vector{M}, c::Constants;
                 AT::Type=TA{Float64}) where M
