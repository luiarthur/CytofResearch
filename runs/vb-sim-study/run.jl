...
if length(ARGS) == 0
  ...
  SEEDS = [2, 4]
  exp_name = "test"
  path_to_data = "../data/simdat-nfac500.bson"
  NITERS = 20000
  BATCHSIZE = 200
else
...
@everywhere include("../vb-cb/imports.jl")
...
@everywhere function fit(seed)
  # Results directory
  results_dir = "results/$(exp_name)/seed_$(seed)"
  mkpath(results_dir)
